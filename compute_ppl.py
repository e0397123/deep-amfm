...
class TextDataset(Dataset):
    def __init__(self, tokenizer, file_path='eval'):
        assert os.path.isfile(file_path)
        directory, filename = os.path.split(file_path)
        cached_features_file = os.path.join(directory, 'cached_lm_' + filename)
...
def load_and_cache_examples(args, tokenizer):
    dataset = TextDataset(tokenizer, file_path=args.eval_data_file)
    return dataset
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if inputs.size(0) > 60: 
            ...
            for x, y in zip(splitted_inputs, splitted_labels):
                x = x.to(args.device)
                y = y.to(args.device)
                with torch.no_grad():
                    outputs = model(x, labels=y)
                    lm_loss = outputs[0]
                    per_sequence_loss +=lm_loss.item() * x.size(0)
            eval_loss += per_sequence_loss / inputs.size(0)
            scores.append(-np.mean(per_sequence_loss/inputs.size(0)))
        else:
    ...
    fn = args.eval_data_file.split(".")[0] + ".fm.prob"
